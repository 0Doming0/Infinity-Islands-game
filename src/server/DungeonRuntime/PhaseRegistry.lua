...
-- The paid-test objective sequence contains 12 mandatory islands.
-- Older migrated Phase01 folders used MaximumIslandCount = 10. Keeping that
-- value caused PaidTestReadinessService to reject a valid teleport and return
-- the player to the Lobby immediately. Clamp legacy content at runtime so
-- already-published GameContent can boot safely before a manual migration pass.
local MINIMUM_OBJECTIVE_ISLAND_COUNT = 12
	...
	if bosses then
		for _, candidate in ipairs(bosses:GetDescendants()) do
			if candidate:IsA("Model")
				and candidate:GetAttribute("BossId") == bossId
			then
				return candidate
			end
		end
...
local function hasArena(islands)
	...
	return arenaFolder
		and (
			arenaFolder:FindFirstChildWhichIsA("Model")
	...
	for _, model in ipairs(enemies:GetChildren()) do
		if model:IsA("Model") and model:GetAttribute("Enabled") ~= false then
			...
			elseif not (
				...
				addError(
					errors,
					model.Name .. " nao possui PrimaryPart ou HumanoidRootPart"
				)
			else
				usable += 1
			end
...
local function validatePhaseFolder(folder)
	...
	if folder:GetAttribute("Enabled") == false
		or string.sub(folder.Name, 1, 1) == "_"
	then
	...
	elseif boss and (
		not boss:FindFirstChildWhichIsA("Humanoid", true)
		or not (
			boss.PrimaryPart
			or boss:FindFirstChild("HumanoidRootPart", true)
		)
	) then
		addError(
			errors,
			"boss "
				.. boss.Name
				.. " precisa de Humanoid e PrimaryPart/HumanoidRootPart"
		)
	end
	...
	local materialName = stringAttribute(
		folder,
		"IslandBlockMaterial",
		"Ground",
		true,
	...
	local textureId = stringAttribute(
		folder,
	...
	if textureId ~= ""
		and not string.match(textureId, "^rbxassetid://%d+$")
	then
		addError(
			errors,
			"IslandBlockTextureId precisa usar rbxassetid://NUMERO ou ficar vazio"
		)
	end
	...
	if imageId ~= "" and not string.match(imageId, "^rbxassetid://%d+$") then
		addError(
			errors,
			"ImageId precisa usar rbxassetid://NUMERO ou ficar vazio"
		)
	end
	...
	local definition = {
		...
		RequiredLevel = math.floor(
			numberAttribute(folder, "RequiredLevel", 1, 1, nil, errors)
		),
		MaxPlayers = math.floor(
			numberAttribute(folder, "MaxPlayers", 4, 1, 4, errors)
		),
		AssetFolder = folder.Name,
		BossId = bossId,
		BaseIslandCount = math.max(
			MINIMUM_OBJECTIVE_ISLAND_COUNT,
			math.floor(
				numberAttribute(
					folder,
					"MaximumIslandCount",
		...
		VictoryCoins = math.floor(
			numberAttribute(folder, "VictoryCoins", 0, 0, nil, errors)
		),
		ImageId = imageId,
		SortOrder = math.floor(
			numberAttribute(folder, "SortOrder", 0, nil, nil, errors)
		),
		Enabled = true,
		IslandBlockColor = color,
		IslandBlockMaterial = materialName,
		IslandBlockTextureId = textureId,
		TextureStudsPerTileU = numberAttribute(
			folder,
			"TextureStudsPerTileU",
			4,
			0.1,
			128,
			errors
		),
		TextureStudsPerTileV = numberAttribute(
			folder,
			"TextureStudsPerTileV",
			4,
			0.1,
			128,
			errors
		),
		MaximumActiveMonsters = math.floor(
			numberAttribute(
				folder,
		...
		EliteReservedMonsterSlots = math.floor(
			numberAttribute(
				folder,
				"EliteReservedMonsterSlots",
				3,
				0,
				20,
				errors
		...
		DefaultMonsterSpawnChance = numberAttribute(
			folder,
			"DefaultMonsterSpawnChance",
			0.72,
			0,
			1,
			errors
		),
		DecorationSpawnChance = numberAttribute(
			folder,
			"DecorationSpawnChance",
			0.7,
			0,
			1,
			errors
	...
	if definition.EliteReservedMonsterSlots
		>= definition.MaximumActiveMonsters
	then
		addError(
			errors,
			"EliteReservedMonsterSlots precisa ser menor que MaximumActiveMonsters"
		)
	end
...
function PhaseRegistry.PublishCatalog()
	...
	local store = DataStoreService:GetDataStore(
		PhaseConfig.CatalogDataStoreName
	)

	local ok, errorMessage = pcall(function()
		store:UpdateAsync(
			PhaseConfig.CatalogDataStoreKey,
			function(previous)
	...
	if not ok then
		warn(
			"[PhaseRegistry] Catalogo do Lobby nao publicado: "
...
function PhaseRegistry.Refresh(options)
	...
	if not phasesFolder then
		warn(
			"[PhaseRegistry] ServerStorage/GameContent/Phases nao existe."
		)
	...
	for _, folder in ipairs(phasesFolder:GetChildren()) do
		if folder:IsA("Folder") then
			...
			if definition then
				if seen[definition.PhaseId]
					or duplicateIds[definition.PhaseId]
				then
					warn(
						"[PhaseRegistry] PhaseId duplicado: "
							.. definition.PhaseId
					)

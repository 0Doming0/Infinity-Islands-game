-- Classifica ilhas geradas.
-- MVP FIX:
-- Combat Islands normais precisam declarar CanSpawnMonster=true.
-- Sem isso Generator.PopulateRuntimeContent nunca chama MonsterSpawner.PopulateIsland.

local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")

local MVPConfig = require(ReplicatedStorage:WaitForChild("MVPConfig"))
local MonetizationCatalog = require(
	ReplicatedStorage:WaitForChild("MonetizationCatalog")
)
...
local function normalizedSeed(value)
	local seed =
		math.floor(math.abs(tonumber(value) or 1))
		% 2147483647
...
local function personalMonetizationMultiplier(
	userId,
	boostName,
	productId
)
	local cleanUserId =
		math.floor(tonumber(userId) or 0)
	...
	local player =
		Players:GetPlayerByUserId(cleanUserId)
	...
	local definition =
		MonetizationCatalog.Get(productId)

	return math.max(
		1,
		tonumber(
			definition
				and definition.ChanceMultiplier
		) or 1
	)
end
...
local function styleIsland(island, islandType, tier)
	...
	if islandType == "Elite" then
		setGrassColor(
			floor,
			grass,
			Color3.fromRGB(108, 54, 56)
		)
		addLabel(
			island,
			floor,
			string.format("ELITE  |  NIVEL %d", tier),
			Color3.fromRGB(255, 96, 78)
		)
	elseif islandType == "Treasure" then
		setGrassColor(
			floor,
			grass,
			Color3.fromRGB(154, 126, 52)
		)
		addLabel(
			island,
			floor,
...
function IslandTypeService.Classify(island, context)
	...
	local roundIndex =
		tonumber(
			context.RoundIndex
				or island:GetAttribute("RoundIndex")
		) or 1

	local role =
		tostring(
			island:GetAttribute("IslandRole")
				or ""
		)
	...
	local tier =
		math.clamp(
			math.floor(
				(roundIndex - 1)
				/ MVPConfig.Difficulty.RoundsPerTier
			) + 1,
			1,
			MVPConfig.Difficulty.MaximumTier
		)

	local baseRewardMultiplier =
		math.max(
			0.1,
			tonumber(
				island:GetAttribute(
	...
	local specialChanceMultiplier =
		math.max(
			0,
			tonumber(
				island:GetAttribute(
					"SpecialIslandChanceMultiplier"
				)
			) or 1
		)

	local generationOwnerUserId =
		tonumber(
	...
	local treasureMonetizationMultiplier =
		personalMonetizationMultiplier(
			generationOwnerUserId,
			"Treasure",
			"TreasureExpedition"
		)

	local eliteMonetizationMultiplier =
		personalMonetizationMultiplier(
			generationOwnerUserId,
			"Elite",
			"EliteExpedition"
	...
	island:SetAttribute(
		"GenerationOwnerUserId",
		generationOwnerUserId > 0
			and generationOwnerUserId
			or nil
	)

	island:SetAttribute(
		"TreasureMonetizationChanceMultiplier",
		treasureMonetizationMultiplier
	)

	island:SetAttribute(
		"EliteMonetizationChanceMultiplier",
		eliteMonetizationMultiplier
	)

	island:SetAttribute("IslandType", "Normal")
	island:SetAttribute(
		"SoloMerchantDecisionVersion",
		SOLO_MERCHANT_DECISION_VERSION
	)
	...
	-- ============================================================
	-- MVP FIX
	-- Toda ilha obrigatoria da rota e uma Combat Island normal.
	-- O Generator verifica este atributo antes de chamar MonsterSpawner.
	-- ============================================================
	if island:GetAttribute("IsMandatoryRoute") == true
		and island:GetAttribute("IsOptionalRoute") ~= true
		...
		Random.new(
			normalizedSeed(
				(
					...
					or context.RoundSeed
					or 1
				)
				+ MVPConfig.SpecialIslands.RandomSalt
			)
		)
	...
	local treasureGap =
		math.max(
			1,
			MVPConfig.SpecialIslands.MinimumTreasureRoundGap
		)

	local treasureWindow =
		roundIndex
			>= MVPConfig.SpecialIslands.MinimumTreasureRound
		and (
			roundIndex
				- MVPConfig.SpecialIslands.MinimumTreasureRound
		) % treasureGap == 0
	...
	if roundIndex
			>= MVPConfig.SpecialIslands.MinimumEliteRound
		and random:NextNumber()
			<= math.clamp(
	...
	then
		...
		island:SetAttribute(
			"RewardMultiplier",
			baseRewardMultiplier
				* MVPConfig.Difficulty.EliteRewardMultiplier
		)
		styleIsland(island, "Elite", tier)
		return "Elite"
	end

	-- Qualquer outra ilha normal tambem pode receber mobs.
	island:SetAttribute("CanSpawnMonster", true)
	island:SetAttribute("CanSpawnItem", false)
